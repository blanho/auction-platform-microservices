...
public class TwoFactorDisabledConsumer : IConsumer<TwoFactorDisabledEvent>
{
    ...
    private readonly ILogger<TwoFactorDisabledConsumer> _logger;

    public TwoFactorDisabledConsumer(
        IIdempotencyService idempotency,
        ITemplateRepository templateRepo,
        INotificationRecordRepository recordRepo,
        IUnitOfWork unitOfWork,
        IEmailSender emailSender,
        ILogger<TwoFactorDisabledConsumer> logger)
    {
    ...
    public async Task Consume(ConsumeContext<TwoFactorDisabledEvent> context)
    {
        var message = context.Message;
        var ct = context.CancellationToken;
        var eventId = $"2fa-disabled-{message.UserId}-{message.DisabledAt:yyyyMMddHHmmss}";

        _logger.LogInformation(
            "Processing TwoFactorDisabledEvent: UserId={UserId}, Username={Username}",
            message.UserId,
            message.Username);
        ...
        var template = await _templateRepo.GetByKeyAsync(NotificationTemplateKeys.TwoFactorDisabled, ct);
        if (template == null || !template.IsActive)
        {
            _logger.LogWarning("Template '2fa-disabled' not found or inactive");
            return;
        }
        ...
        var data = new Dictionary<string, string>
        {
            ...
            [NotificationTemplateDataKeys.DisabledAt] = message.DisabledAt.ToString("f")
        };

        var subject = TemplateHelper.RenderTemplate(template.Subject ?? "Two-Factor Authentication Disabled", data);
        ...
        var record = NotificationRecord.Create(
            ...
            NotificationTemplateKeys.TwoFactorDisabled,
        ...
        try
        {
            ...
            if (result.Success)
            {
                ...
                _logger.LogInformation("2FA disabled notification sent to {Email}", message.Email);
            }
            else
            {
