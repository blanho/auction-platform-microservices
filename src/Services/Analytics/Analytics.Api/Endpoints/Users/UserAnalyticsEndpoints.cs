...
using Analytics.Application.Features.UserAnalytics.GetQuickStats;
using Analytics.Application.Features.UserAnalytics.GetSellerAnalytics;
using Analytics.Application.Features.UserAnalytics.GetTrendingSearches;
using Analytics.Application.Features.UserAnalytics.GetUserDashboardStats;
using BuildingBlocks.Web.Authorization;
using BuildingBlocks.Web.Helpers;

namespace Analytics.Api.Endpoints.Users;
...
public class UserAnalyticsEndpoints : ICarterModule
{
    ...
    private static async Task<Ok<TrendingSearchesResponse>> GetTrendingSearches(
        ...
        CancellationToken cancellationToken)
    {
        var searches = await sender.Send(
            new GetTrendingSearchesQuery(limit ?? AnalyticsDefaults.DefaultLimit),
            cancellationToken);
