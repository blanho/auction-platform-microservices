...
public class AuthEndpoints : ICarterModule
{
    ...
    private static async Task<IResult> RefreshToken(HttpContext httpContext, IConfiguration configuration, ISender sender, CancellationToken cancellationToken)
    {
        ...
        if (!result.IsSuccess)
        {
            ...
            {
                var problemDetails = ProblemDetailsHelper.FromError(result.Error);
                return Results.Json(
                    new { code = "security_termination", message = problemDetails.Detail },
    ...
    private static async Task<IResult> Logout(ClaimsPrincipal user, HttpContext httpContext, IConfiguration configuration, ISender sender, CancellationToken cancellationToken)
    {
    ...
    private static async Task<IResult> LogoutAll(ClaimsPrincipal user, HttpContext httpContext, IConfiguration configuration, ISender sender, CancellationToken cancellationToken)
    {
        var userId = user.GetRequiredUserIdString();
    ...
    private static async Task<IResult> GetCurrentUser(ClaimsPrincipal user, ISender sender, CancellationToken cancellationToken)
    {
        var userId = user.GetRequiredUserIdString();
